...
from pc_params import seriesParams, dimensions, generate_description, all_params
...
def generate_one_footprint(motel, params, configuration):
    ...
    kicad_mod.append(PadArray(initial=1, start=[0, 0],
        ...
        **optional_pad_params))
    #Footprint has three rows of Pins
    kicad_mod.append(PadArray(initial=1, start=[0, -5.08],
        x_spacing=params.pin_pitch, pincount=params.num_pins,
        size=[params.pin_Sx, params.pin_Sy], drill=seriesParams.drill,
        type=Pad.TYPE_THT, shape=Pad.SHAPE_OVAL, layers=configuration['pin_layers'],
        **optional_pad_params))
    kicad_mod.append(PadArray(initial=1, start=[0, -10.16],
        x_spacing=params.pin_pitch, pincount=params.num_pins,
        size=[params.pin_Sx, params.pin_Sy], drill=seriesParams.drill,
        type=Pad.TYPE_THT, shape=Pad.SHAPE_OVAL, layers=configuration['pin_layers'],
        **optional_pad_params))
    ...
    if params.angled:
        ...
        side_recess_left_translation = Translation(body_top_left[0] + 1 + (seriesParams.flange_lenght if params.flanged else 0), 4)
        side_recess_left_translation.append(PolygoneLine(polygone=side_recess_poly,  layer="F.SilkS", width=configuration['silk_line_width']))
        side_recess_right_translation = Translation(body_bottom_right[0] - 3.3 - (seriesParams.flange_lenght if params.flanged else 0),4)
        side_recess_right_translation.append(PolygoneLine(polygone=side_recess_poly,  layer="F.SilkS", width=configuration['silk_line_width']))
        
        if configuration['inner_details_on_fab']:
            side_recess_left_translation.append(PolygoneLine(polygone=side_recess_poly,  layer="F.Fab", width=configuration['fab_line_width']))
            side_recess_right_translation.append(PolygoneLine(polygone=side_recess_poly,  layer="F.Fab", width=configuration['fab_line_width']))
        kicad_mod.append(side_recess_left_translation)
        kicad_mod.append(side_recess_right_translation)
        
        for i in range(params.num_pins - 1):
            inter_pin_recess_translation = Translation(i*params.pin_pitch + params.pin_pitch / 2, 0)
            inter_pin_recess_poly=[
                {'x': -1.15, 'y':body_top_left[1]},
                {'x': -1.15, 'y':body_bottom_right[1] - 15.6},
                {'x': -2.3, 'y':body_bottom_right[1] - 15.6},
                {'x': -2.3, 'y':body_bottom_right[1] - 3.5},
                {'x': 2.3, 'y':body_bottom_right[1] - 3.5},
                {'x': 2.3, 'y':body_bottom_right[1] - 15.6},
                {'x': 1.15, 'y':body_bottom_right[1] - 15.6},
                {'x': 1.15, 'y': body_top_left[1]}
            ]
            inter_pin_recess_translation.append(PolygoneLine(polygone=inter_pin_recess_poly, layer='F.SilkS', width=configuration['silk_line_width']))
            if configuration['inner_details_on_fab']:
                inter_pin_recess_translation.append(PolygoneLine(polygone=inter_pin_recess_poly,  layer="F.Fab", width=configuration['fab_line_width']))
            kicad_mod.append(inter_pin_recess_translation)
    else:
    ...
    ################################################## Courtyard ##################################################
    if params.angled:
        crtyd_top_left=v_offset(silk_top_left, configuration['courtyard_offset']['connector'])
    else:
    ...
    if not params.angled:
        ...
        if configuration['with_fab_layer']:
            kicad_mod.append(PolygoneLine(
                polygone=create_pin1_marker_triangle(bottom_y = -12,
                    dimensions = [1.9, -body_top_left[1]-12], with_top_line = False),
                layer='F.Fab', width=configuration['fab_line_width']))
